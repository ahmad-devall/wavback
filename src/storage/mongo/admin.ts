...
export class AdminStorage implements AdminRepo {
    ...
    async findAll(query: Object): Promise<IAdmin[]> {
        try {
            const dbObj = await Admin.find({ ...query })
            return dbObj
        } catch (error) {
    ...
    async updateMany(query: Object, payload: IAdmin): Promise<Object> {
        try {
            const db_res = await Admin.updateMany(query, payload)
